...
\title{\bf { AMATH 586 Spring 2021 \\ Homework 2 ---
Due April 23 on GitHub by 11pm} }
\maketitle
...
\item Explicit solution of leapfrog:  Consider the difference equation
  \begin{align*}
    U^{n+1} &+ U^{n-1} = 2\lambda U^n, \quad n \geq 0,\\
    U^0 &= 1, \quad U^{-1} = 0.
  \end{align*}
  Provided that $-1 \leq \lambda \leq 1$ perform the following steps:
...
\item Argue that this can be replaced with
 ...
 \item With $V^n$ known, $V^n = U^n - e^{i \theta} U^{n-1}$ gives an inhomogeneous recurrence relation for $U^n$.  Find a formula for $U^n$. For which value(s) of $\lambda \in [-1,1]$ is $U^n$ largest?
\end{enumerate} 
